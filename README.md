...
프로젝트 로컬 Hermes hook 플러그인입니다. 개인정보 보호를 고려한 lifecycle 및 검증 메타데이터를 `.hermes/audit.db`(SQLite)에 기록합니다. Workflow evidence를 관찰하고 검증 알림을 제공하지만 Checkstyle, JUnit, ArchUnit, Spring Modulith 검증 또는 CI를 대체하지 않습니다.
...
- `.hermes/README.md`: 프로젝트 수준 활성화 및 capability 설정
...
`python .hermes/plugins/agent-audit/test_metadata.py`로 metadata 일치를 확인합니다.
...
python .hermes/plugins/agent-audit/test_shell_hook.py
python .hermes/plugins/agent-audit/test_metadata.py
```

Desktop/TUI/dashboard chat은 Hermes shell hook을 사용합니다. Profile에는 `post_tool_call`, `on_skill_lifecycle`, `pre_verify`, `on_session_end` hook entry가 있어야 하며, 각 entry는 `shell_hook.py`를 가리켜야 합니다. Python plugin hook만으로는 CLI/Gateway에서만 동작합니다.
